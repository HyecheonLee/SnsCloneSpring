...
object SseEmitterUtils {
    ...
    fun createSseEmitter(type: SseEvent.EventType, key: String? = null) = run {
        ...
        if (!sseEvent.connectionCheck()) throw RuntimeException("")
        events.add(sseEvent)
        emitter.onTimeout {
    ...
    fun <T> eventSend(eventMessage: EventMessage<T>) {
        val fails = events.filter { it.sendAble(eventMessage) }.filter { !it.eventSend(eventMessage) }
        fails.forEach { events.remove(it) }
    }
